...
function findMatchingBrace(text, openIndex) {
  ...
  throw new Error('V13: could not find matching brace');
}

function replaceFunction(source, signature, replacement) {
  const start = source.indexOf(signature);
  if (start < 0) throw new Error('V13 anchor not found: ' + signature);
  const open = source.indexOf('{', start);
  if (open < 0) throw new Error('V13 opening brace not found: ' + signature);
