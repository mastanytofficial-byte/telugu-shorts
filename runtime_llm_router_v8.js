  ...
  for (const p of providers) {
    try {
      ...
      if (p.kind === 'gemini') {
        ...
        const requestBody = { contents: [{ parts: [{ text: prompt }] }], generationConfig: { temperature: 0.25, maxOutputTokens: 1800, thinkingConfig: { thinkingBudget: 0 } } };
        const res = await fetchWithTimeout(url, { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-goog-api-key': p.key }, body: JSON.stringify(requestBody) }, 30000);
...
const factReplacement = `async function getOrGrowFactOutline(category, discoveredFacts, usedTopics) {
  ...
  const maxAttempts = 6;
  let bestCandidate = null, bestScore = -1, bestTopic = null;
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    const topicPrompt = 'You are selecting a topic for a Telugu Amazing Facts Shorts channel. Category: ' + category + '. Generate ONE specific, established, surprising topic that is NOT among these previously used topics/facts: ' + seen.slice(-12000) + '. Do not repeat or paraphrase a previous topic. Return only the topic name in English, 2-6 words.';
    ...
    try {
      ...
      if (saturation > 50) { log('  Fact is oversaturated; trying another fresh topic.'); continue; }
