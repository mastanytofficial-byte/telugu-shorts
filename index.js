...
// Topic/Script (Gemini) -> Voice (Google TTS) -> Images/Video (Pexels/AI) -> Music (Jamendo) -> Video (FFmpeg) -> YouTube
...
// A stalled connection to any external API (Gemini, Google TTS, Pexels,
...
// Asks Gemini for one new, high-confidence fact outline in the given
...
// Independent second pass: asks Gemini to specifically critique the
...
// fact about it from Gemini, retrying up to 6 times (a new topic each
...
async function getOrGrowFactOutline(category, discoveredFacts, usedTopics) {
  ...
  // past the first few, mostly just multiplying Gemini calls and rate-limit
  ...
  // Gemini calls / rate-limit risk) for no real quality gain.
...
// between periods) that's too long. Gemini is prompted to add periods between
...
function buildNarrationPrompt(category, recentTitles, outline, beats) {
  ...
  return `కింద ఇచ్చిన JSON లో story beats ఉన్నాయి (ఇదే క్రమంలో వాడు, కొత్తవి కల్పించకు):
${beatsJSON}

(గ్రౌండింగ్ కోసం అసలైన fact: ${compactOutlineForGrounding(outline)})

నువ్వు ఒక professional YouTube Shorts storyteller వి — ఈ beats ని తెలుగు narration గా విస్తరించు. Personal scenario తో మొదలుపెట్టు (viewer ప్రతిరోజూ చేసేదానితో connect చేయి), assumption→correction technique వాడు ("...అనుకుంటున్నారా? కాదు..."), ప్రతి సంఖ్యకీ ఒక పోలిక ఇవ్వు, consequence తో ముగించు. ప్రతి వాక్యంలో ఒక్క idea మాత్రమే, "..." తో pause.

నియమాలు:
- **ఖచ్చితంగా ${min}-${max} తెలుగు పదాలు**.
- పూర్ణవిరామం (.) ఎప్పుడూ వాక్యం మధ్యలో, క్రియకి ముందు వద్దు — "..." pause కి, period వాక్యాంతానికే.
- సంఖ్యలు 1000+ ఉంటే తెలుగు మాటల్లోనే (10000 → "పది వేలు"). 1000 కన్నా తక్కువ అంకెల్లో వాడొచ్చు.
- తెలుగు లిపిలోనే — Romanized వద్దు. CTA రాయకు.${avoidLine}

**కేవలం తెలుగు narration టెక్స్ట్ మాత్రమే ఇవ్వు — టైటిల్ వద్దు, లేబుల్స్ వద్దు, మార్క్‌డౌన్ వద్దు, ఇంకేమీ ముందు/వెనుక రాయకు.**`;
}
...
ఖచ్చితంగా ఈ 3-లైన్ల ఫార్మాట్‌లో మాత్రమే ఇవ్వు, మరేమీ రాయకు:
TITLE: (5-8 తెలుగు పదాల్లో ఒక చిన్న శీర్షిక, emoji వద్దు)
...
HOOK: (ఈ narration లోని ప్రధాన ప్రశ్నని, 15 తెలుగు పదాల లోపు రాయి, emoji వద్దు)`;
}
...
// Uses the auto-updating "gemini-flash-latest" ALIAS (not a pinned version
// number) specifically to avoid a repeat of a real failure: our originally
// pinned "gemini-2.5-flash" started returning 404 "no longer available to
// new users" within about a month, because Google cycled through
// 2.5→3.5→3.6 in rapid succession. This alias always tracks Google's
// current recommended GA Flash model, so our unattended daily pipeline
// won't silently break again the next time Google ships a new generation.
const GEMINI_PRIMARY_MODEL = 'gemini-flash-latest';
// Fallback pinned to a specific current GA lite model rather than an
// alias — lite-tier models have historically had longer support windows,
// and having primary+fallback both auto-update would remove the
// resilience benefit of a fallback (they'd always be on the same
// generation, deprecating together).
const GEMINI_FALLBACK_MODEL = 'gemini-3.5-flash-lite';
let primaryModelExhaustedThisRun = false;

async function callLLM(prompt, attempt = 1, model = (primaryModelExhaustedThisRun ? GEMINI_FALLBACK_MODEL : GEMINI_PRIMARY_MODEL)) {
  ...
  if (!data.candidates || !data.candidates[0] || !data.candidates[0].content) {
    ...
    const isRateLimit = res.status === 429 || status === 'RESOURCE_EXHAUSTED';
    const isDailyLimit = /per day|daily|RPD/i.test(errorMessage);

    if (isRateLimit && isDailyLimit) {
      if (model === GEMINI_PRIMARY_MODEL) {
        // Primary model's daily budget is exhausted — try the fallback
        // model, a separate free daily quota. Remember this for the rest
        // of the run so future independent calls skip straight to the
        // fallback instead of re-discovering the same exhaustion each time.
        primaryModelExhaustedThisRun = true;
        log(`WARNING: "${GEMINI_PRIMARY_MODEL}" daily limit reached — switching to fallback model "${GEMINI_FALLBACK_MODEL}" for the rest of this run.`);
        return callLLM(prompt, 1, GEMINI_FALLBACK_MODEL);
      }
      throw new Error(`Gemini daily limit reached on both "${GEMINI_PRIMARY_MODEL}" and fallback "${GEMINI_FALLBACK_MODEL}" — this run cannot continue today. ${errorMessage}`);
    }
    ...
    throw new Error('Gemini did not return content: ' + JSON.stringify(data));
  }

  const parts = data.candidates[0].content.parts || [];
  let content = parts.map(p => p.text || '').join('').trim();
  // Defensive safety net: strip any stray <think>...</think> block, in
  // case a future model swap brings a reasoning model that leaks planning
  // text the way gpt-oss-120b did on Groq (our previous provider).
  content = content.replace(/<think>[\s\S]*?<\/think>/gi, '').trim();
  if (!content) {
    throw new Error('Gemini returned an empty response (finishReason: ' + (data.candidates[0].finishReason || 'unknown') + ')');
  }
...
{"hook": "...", "question": "...", "reveal": "...", "twist": "...", "ending": "..."}`;
  ...
  // One retry with a stricter, shorter reminder before accepting the
  // degraded single-beat fallback — a malformed reply here collapses
  // Stage-2 into near one-beat narration (only "hook" populated), which is
  // a much bigger quality hit than the cost of one extra Gemini call.
  if (!beats) {
...
// stray text or ```json fences around it, since Gemini models don't always
...
async function generateContent(category, recentTitles, outline, ctaSentence) {
  log(`Generating ${category} content via Gemini...`);
...
నియమాలు: JSON లోని beats క్రమాన్నే విస్తరించి చెప్పు, కొత్త కల్పితం/సంఖ్యలు జోడించకు. Personal scenario తో connect చేయి, assumption→correction technique వాడు ("...అనుకుంటున్నారా? కాదు..."), ప్రతి సంఖ్యకీ ఒక comparison ఇవ్వు, consequence తో ముగించు. ఖచ్చితంగా తెలుగు లిపిలోనే రాయి (Romanized వద్దు). "..." తో dramatic pause ఇస్తూ, 8 పదాల లోపు చిన్న వాక్యఖండాలతో, న్యూలైన్‌లతో రాయి (ఉదా: "కానీ...\nఅది నిజం కాదు!") — flat, పొడవైన వాక్యాలు వద్దు. **Period (.) ఎప్పుడూ క్రియకి ముందు/వాక్యం మధ్యలో పెట్టకు — పూర్తి ఆలోచన ముగిసిన చోటే.** CTA/emoji/టైటిల్/లేబుల్స్ వద్దు — కేవలం narration టెక్స్ట్ మాత్రమే ఇవ్వు.`;
  ...
  // CALL B: metadata (title/keywords/hook) extracted from the FINISHED,
  // validated narration — a much simpler task than generating everything
  // at once.
...
// FALLBACK ONLY: used if Gemini ever fails to return a usable TITLE line.
...
// Lenient about numbering style (1. / 1) / **1.**) since Gemini varies this.
...
...`;
  ...
  const raw = await callLLM(prompt);
  log(`Raw sentence-keywords/scenes response from Gemini:\n${raw}`);
...
// part comes from Gemini's HOOK field, kept separate from the spoken SCRIPT.
...
async function main() {
  ...
  checkSecret('GEMINI_API_KEY', GEMINI_API_KEY);
