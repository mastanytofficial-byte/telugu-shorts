...
async function getOrGrowFactOutline(category, categoryRunCounts, discoveredFacts) {
  ...
  // A candidate scoring at or above this is accepted immediately (early
  // exit, saves the remaining attempts). Below it, we keep the BEST
  // scoring candidate found and use it once maxAttempts is exhausted,
  // rather than an unbounded "keep trying until 95+" loop that could spin
  // forever if the model rarely self-scores that high.
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    try {
      const candidate = await generateNewFactOutline(category, previouslyDiscovered);
      if (candidate) {
...
- సంఖ్యలు (వాడితే) ఎప్పుడూ తెలుగు మాటల్లోనే (2000 → "రెండు వేలు"), అంకెల్లో వద్దు — **తప్ప, ఉదాహరణలో చూపినట్టు చిన్న సంఖ్యలు (300, 206 వంటివి) అంకెల్లోనే వాడొచ్చు, ఇది TTS కి సహజంగా వినిపిస్తుంది.**
...
నీ అత్యంత ముఖ్యమైన లక్ష్యం audience retention. ప్రతి 5-8 సెకన్లకూ ఒక కొత్త curiosity/surprise పరిచయం చేయి. వినేవారికి ఎప్పుడూ "అసలైన reveal ఇంకా రావాల్సి ఉంది" అనే feeling ఉండాలి. energy ఎప్పుడూ తగ్గకూడదు — 80%+ audience retention సాధించేలా రాసినట్టు రాయి.${avoidLine}`;
...
async function generateAudioForScript(sentences) {
  const commaGap = 0.08;    // shortest pause, at a comma
  const ellipsisGap = 0.22; // medium "dramatic pause", at "..."
  const periodGap = 0.35;   // longest pause, between sentences
