...
// fact from Groq first for every video (per explicit user decision — they
// want real-world variety, not repeats from a fixed list, and understand
// the residual accuracy risk: an LLM "recalling" a fact isn't the same as
// a verified database lookup, even with self-verification as a safety
// net). Falls back to the curated+discovered bank only if generation or
// verification fails, so a bad Groq response never blocks a run. Verified
// fresh facts are still saved permanently, so the fallback bank keeps
// growing too.
async function getOrGrowFactOutline(category, categoryRunCounts, discoveredFacts) {
  ...
  log(`Attempting a fresh, self-verified fact for "${category}" (bank of ${bank.length} available as fallback).`);
  try {
    const candidate = await generateNewFactOutline(category, bank);
    if (candidate) {
      const isVerified = await verifyFactOutline(candidate);
      if (isVerified) {
        log(`  Fresh fact VERIFIED — using it for this video.`);
        return { outline: candidate, newlyDiscovered: candidate };
      }
    }
  } catch (e) {
    log(`  WARNING: fresh-fact generation failed (${e.message}) — falling back to the existing bank.`);
  }
  log(`  Fresh generation didn't produce a usable, verified fact this time — using the existing bank instead.`);

  // Fallback: the FULL combined bank (curated + discovered), indexed so
  // every entry gets its turn without repeats.
