...
function buildPrompt(category, recentTitles, outline, beats) {
  ...
  // Kept deliberately lean: the JSON beats already carry the story shape
  // (hook → question → reveal → twist → ending), so this prompt only needs
  // to say HOW to expand each beat and state the hard formatting rules —
  // it doesn't need to re-teach "what a good story looks like" from
  // scratch with a full before/after example, which was making this one
  // prompt do too much at once and contributing to malformed/short Groq
  // responses.
...
HOOK: (JSON లోని hook భావననే, 15 తెలుగు పదాల లోపు ప్రశ్నగా రాయి — video description లో వాడతాం. ఇందులో emoji వాడకు, మేమే జోడిస్తాం.)
SCRIPT: (పైన చెప్పిన నియమాల ప్రకారం పూర్తి వాయిస్-ఓవర్ టెక్స్ట్ — ఇందులో emoji లు వాడకు. అదనంగా, స్క్రిప్ట్‌లో ఈ structural markers ని సంబంధిత భాగం ముందు పెట్టు: [HOOK] hook వాక్యానికి, [VISUAL] reveal భాగం మొదలయ్యే చోట, [TWIST] twist భాగం మొదలయ్యే చోట. ఇవి మేము స్క్రిప్ట్ ని విభాగాలుగా గుర్తించడానికి వాడతాం, TTS కి పంపేముందు తీసేస్తాం.)`;
}
...
async function generateContent(category, recentTitles, outline, ctaSentence) {
  ...
  for (let retryAttempt = 1; retryAttempt <= 2 && (wordCount < target.min - 15 || bestWordCount > MAX_SAFE_WORDS); retryAttempt++) {
    ...
    const retryPrompt = tooLong
      ? `కింద ఇచ్చిన JSON story beats నుండి తెలుగు narration రాయి — ఖచ్చితంగా ${target.min}-${target.max} తెలుగు పదాలు ఉండాలి (మీ మునుపటి ప్రయత్నం ${bestWordCount} పదాలు వచ్చింది, ఇది చాలా ఎక్కువ — వీడియో 80 సెకన్ల లోపు ఉండాలి). ముఖ్యమైన భాగాలనే ఉంచి, సంక్షిప్తంగా చెప్పు:\n\n${beatsJSON}\n\nనియమాలు: JSON లోని beats క్రమాన్నే వాడు, కొత్త కల్పితం జోడించకు. ఖచ్చితంగా తెలుగు లిపిలోనే రాయి. "..." తో dramatic pause ఇస్తూ, 8 పదాల లోపు చిన్న వాక్యఖండాలతో రాయి. CTA/emoji వద్దు.\n\nజవాబు ఫార్మాట్:\nTITLE: (5-8 పదాల శీర్షిక, emoji వద్దు)\nKEYWORDS: (3 నిర్దిష్ట ఆంగ్ల keywords)\nHOOK: (hook ప్రశ్న, 15 పదాల లోపు, emoji వద్దు)\nSCRIPT: (సంక్షిప్త వాయిస్-ఓవర్ టెక్స్ట్, emoji/CTA వద్దు)`
      : `కింద ఇచ్చిన JSON story beats నుండి తెలుగులో వివరణాత్మకమైన narration రాయి — ఖచ్చితంగా ${target.min}-${target.max} తెలుగు పదాలు ఉండాలి (మీ మునుపటి ప్రయత్నం కేవలం ${wordCount} పదాలు మాత్రమే వచ్చింది, ఇది చాలా తక్కువ, padding చేయకుండా ఈ beats ని పూర్తిగా వివరించి ఈ నిడివికి చేరుకో):
