...
// Topic/Script (Cerebras) -> Voice (Google TTS) -> Images/Video (Pexels/AI) -> Music (Jamendo) -> Video (FFmpeg) -> YouTube
...
// A stalled connection to any external API (Cerebras, Google TTS, Pexels,
...
// Asks Cerebras for one new, high-confidence fact outline in the given
...
// Independent second pass: asks Cerebras to specifically critique the
...
// fact about it from Cerebras, retrying up to 6 times (a new topic each
...
async function getOrGrowFactOutline(category, discoveredFacts, usedTopics) {
  ...
  // past the first few, mostly just multiplying Cerebras calls and rate-limit
  ...
  // Cerebras calls / rate-limit risk) for no real quality gain.
...
// between periods) that's too long. Cerebras is prompted to add periods between
...
- పూర్ణవిరామం (.) ఎప్పుడూ వాక్యం మధ్యలో, క్రియకి ముందు, లేదా ఒక పదబంధం మధ్యలో వద్దు — "..." pause కి, period వాక్యాంతానికే. తప్పు ఉదాహరణ: "అన్ని రకాల ఆహార. పదార్థాలు కొద్ది రోజుల్లో..." (ఇక్కడ "ఆహార పదార్థాలు" ఒకే పదబంధం, మధ్యలో period తప్పు). ప్రతి వాక్యం పూర్తి, అర్థవంతమైన ఆలోచనతోనే ముగియాలి.
...
// Verified Cerebras's actual current public-endpoint catalog (only 3
// models total, confirmed via a source describing it as their "static
// three-model catalog"): zai-glm-4.7, gpt-oss-120b, gemma-4-31b.
// llama-4-scout (our first attempt) wasn't in this list at all, causing a
// model_not_found error. Using gemma-4-31b as primary — no deprecation
// flag, and explicitly described as Cerebras's "current recommendation
// for fresh onboarding". gpt-oss-120b as fallback — its known <think>
// reasoning-leak quirk (from our own earlier experience on Groq) is
// already handled by the defensive stripping below, so we're not going in
// blind on this one. Deliberately avoiding zai-glm-4.7: scheduled for
// deprecation on August 17, 2026 — only days away from today.
const CEREBRAS_PRIMARY_MODEL = 'gemma-4-31b';
const CEREBRAS_FALLBACK_MODEL = 'gpt-oss-120b';
let primaryModelExhaustedThisRun = false;

async function callLLM(prompt, attempt = 1, model = (primaryModelExhaustedThisRun ? CEREBRAS_FALLBACK_MODEL : CEREBRAS_PRIMARY_MODEL)) {
  ...
  if (!data.choices || !data.choices[0]) {
    ...
    const errorType = data.error && data.error.type;
    const isRateLimit = res.status === 429 || errorType === 'rate_limit_exceeded' || errorType === 'too_many_requests';
    const isDailyLimit = /per day|daily|RPD|quota/i.test(errorMessage);

    if (isRateLimit && isDailyLimit) {
      if (model === CEREBRAS_PRIMARY_MODEL) {
        primaryModelExhaustedThisRun = true;
        log(`WARNING: "${CEREBRAS_PRIMARY_MODEL}" daily limit reached — switching to fallback model "${CEREBRAS_FALLBACK_MODEL}" for the rest of this run.`);
        return callLLM(prompt, 1, CEREBRAS_FALLBACK_MODEL);
      }
      throw new Error(`Cerebras daily limit reached on both "${CEREBRAS_PRIMARY_MODEL}" and fallback "${CEREBRAS_FALLBACK_MODEL}" — this run cannot continue today. ${errorMessage}`);
    }
    ...
    if (isRateLimit && attempt <= 3) {
      ...
      log(`WARNING: Cerebras rate limit hit (attempt ${attempt}/3) — waiting ${waitMs / 1000}s before retrying.`);
      await sleep(waitMs);
      return callLLM(prompt, attempt + 1, model);
    }
    throw new Error('Cerebras did not return content: ' + JSON.stringify(data));
  }

  let content = (data.choices[0].message.content || '').trim();
  // Defensive safety net: strip any stray <think>...</think> block, in
  // case a reasoning-style model leaks planning text the way gpt-oss-120b
  // did on Groq (our earlier provider).
  content = content.replace(/<think>[\s\S]*?<\/think>/gi, '').trim();
  if (!content) {
    throw new Error('Cerebras returned an empty response (finish_reason: ' + (data.choices[0].finish_reason || 'unknown') + ')');
  }
...
{"hook": "...", "question": "...", "reveal": "...", "twist": "...", "ending": "..."}`;
  ...
  // a much bigger quality hit than the cost of one extra Cerebras call.
  if (!beats) {
...
// stray text or ```json fences around it, since Cerebras models don't always
...
async function generateContent(category, recentTitles, outline, ctaSentence) {
  log(`Generating ${category} content via Cerebras...`);
...
// FALLBACK ONLY: used if Cerebras ever fails to return a usable TITLE line.
...
// Lenient about numbering style (1. / 1) / **1.**) since Cerebras varies this.
...
...`;
  ...
  const raw = await callLLM(prompt);
  log(`Raw sentence-keywords/scenes response from Cerebras:\n${raw}`);
...
// part comes from Cerebras's HOOK field, kept separate from the spoken SCRIPT.
...
async function main() {
  ...
  checkSecret('CEREBRAS_API_KEY', CEREBRAS_API_KEY);
