...
// Switched from Groq's gpt-oss-120b/20b to Gemini's OpenAI-compat endpoint
...
తప్పు: హుక్ లో "ఎలెక్ట్రిక్ ఈల్స్ తమ స్వంత విద్యుత్తుతో అంధులైపోతాయా?" అని అడిగి, తర్వాతి Reveal/Detail/Twist/Ending అంతా వాటి voltage/shock power గురించి మాత్రమే చెప్పి, "అంధులైపోవడం" (blindness) ప్రస్తావననే లేకుండా వదిలేయడం (Reveal హుక్ కి నిజమైన జవాబు కావాలనే నియమం పాటించలేదు) | సరైనది: హుక్ ని Reveal నిజంగా వివరించే విషయానికే (ఇక్కడ: ఎంత శక్తివంతమైన voltage తయారుచేస్తాయో) సరిపోయేలా రాయాలి — "ఎలెక్ట్రిక్ ఈల్స్ ఎంత శక్తివంతమైన విద్యుత్తు తయారుచేస్తాయో మీకు తెలుసా?" లాంటిది, Reveal అందులో ప్రస్తావించని కొత్త claim (blindness) హుక్ లో పెట్టకూడదు.
...
async function request(url,options,original,model,repairReason=''){let b=JSON.parse(String(options.body||'{}'));b.model=model;b.messages=[{role:'user',content:makePrompt(original,repairReason)}];b.temperature=.15;b.max_tokens=2500;b.response_format={type:'json_schema',json_schema:{name:'telugu_fact_narration',strict:true,schema:{type:'object',properties:{hook:{type:'string'},buildup:{type:'string'},reveal:{type:'string'},detail:{type:'string'},twist:{type:'string'},ending:{type:'string'}},required:['hook','buildup','reveal','detail','twist','ending'],additionalProperties:false}}};const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),45000);let r;try{r=await PREVIOUS(url,{...options,signal:controller.signal,body:JSON.stringify(b)});}catch(e){if(e.name==='AbortError'){const te=new Error('Gemini request timed out after 45000ms (per-attempt cap, independent of the outer request-level timeout)');te.status=598;throw te;}throw e;}finally{clearTimeout(timer);}let d;try{d=await r.clone().json();}catch{throw new Error(`Gemini returned non-JSON response (HTTP ${r.status})`);}if(!r.ok||d?.error){const e=new Error(`Gemini API error ${r.status}: ${d?.error?.message||d?.error?.code||'unknown'}`);e.status=r.status;e.retryAfter=Number(r.headers.get('retry-after')||0);throw e;}return{r,d};}
