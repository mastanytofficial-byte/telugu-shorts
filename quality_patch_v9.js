...
async function generateNewFactOutline(category, topic, existingOutlines) {
  const recent = existingOutlines.slice(-18).map((x, i) => `\${i + 1}. \${x.slice(0, 220)}`).join('\\n');
  const prompt = `You are the FACT RESEARCHER for a Telugu Amazing Facts Shorts channel.
...
const verifyFn = String.raw`async function verifyFactOutline(outline) {
  const prompt = `You are a strict fact-checker for a Telugu facts channel.
...
const growFn = String.raw`async function getOrGrowFactOutline(category, discoveredFacts, usedTopics) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    const topic = pickTopic(category, [...used, ...candidates.map(c => c.topic)]);
    log(`Attempting a fresh high-quality fact for "\${category}" / "\${topic}" (attempt \${attempt}/\${maxAttempts})...`);
    try {
      const outline = await generateNewFactOutline(category, topic, [...previous, ...candidates.map(c => c.outline)]);
      if (!outline) { log('  Candidate generation returned UNSURE/malformed — trying another topic.'); continue; }

      const normalize = text => text.toLowerCase().replace(/[^\\p{L}\\p{N}\\s]/gu, ' ').split(/\\s+/).filter(w => w.length >= 3).slice(0, 80);
      const currentTokens = new Set(normalize(outline));
      const tooSimilar = [...previous, ...candidates.map(c => c.outline)].some(old => {
        const oldTokens = new Set(normalize(old));
        if (!currentTokens.size || !oldTokens.size) return false;
        let overlap = 0;
        for (const t of currentTokens) if (oldTokens.has(t)) overlap++;
      ...
      candidates.push({ outline, topic, score: checked.score });
      if (!best || checked.score > best.score) best = { outline, topic, score: checked.score };
      if (checked.score >= scoreBar) return { outline, newlyDiscovered: outline, topic };
...
const contentFn = String.raw`async function generateContent(category, recentTitles, outline, ctaSentence) {
  ...
  const prompt = `Write the FINAL spoken narration for a Telugu Amazing Facts YouTube Short.
...
Return ONLY the narration text.`;
  ...
  script = splitIntoSentences(script).filter(x => !/సబ్.?స్క్రైబ్|subscribe/i.test(x)).join('\\n').trim();
