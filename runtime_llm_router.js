...
function getBody(options) {
  try { return JSON.parse(String(options.body || '{}')); } catch (_) { return null; }
}
...
function frozenGroqResponse(original) {
  return makeResponseLike(original, {
    choices: [{ message: { role: 'assistant', content: frozenNarration }, finish_reason: 'stop' }]
  });
}
...
global.fetch = async function protectedFetch(url, options = {}) {
  ...
  // Generic helper calls in index.js currently request a large completion
  // budget even when they only need a few lines. That requested budget counts
  // toward Groq TPM and was causing rate-limit failures before narration was
  // reached. Keep helpers deliberately small; the V14 guard owns the larger
  // completion budget for the final narration.
